...
expression: 
  ...
  | array '$' ID
    ...
    | ID '$' array {
      ...
      //verify stencil exists
      symbol* stencil = lookup(tds, $1);
      if (stencil == NULL) {
        print_error("Unknown ID", $1);
        free($1);
        return 0;
      }
      if (stencil->type != STENCIL) {
        print_error($1, "is not a stencil");
        free($1);
        return 0;
      }
      if (DEBUG) printf("checkpoint %d \n", checkpoint++);
      int array_line_size = get_nth_dim($3.base->array.nb_dim, $3.base->array.dim_list);
      int stenc_x = get_nth_dim(1, stencil->array.dim_list);
      int min_i = -stenc_x;
      int max_i = stenc_x;
      if (DEBUG) printf("checkpoint %d \n", checkpoint++);
      ...
      symbol* temp1 = new_temp(&tds);
      quad* q = quad_geni(Q_MULTI, $3.offset, 4, temp1);
      $$.code = concat_quad($$.code , q);
      symbol* array_base_addr = new_temp(&tds);
      q = quad_gen(Q_GET_A, $3.base, NULL, array_base_addr);
      $$.code = concat_quad($$.code, q);
      if (DEBUG) printf("checkpoint %d \n", checkpoint++);
